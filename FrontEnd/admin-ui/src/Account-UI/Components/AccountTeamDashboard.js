...
const AccountTeamDashboard = () => {
  ...
  const [completedRequests, setCompletedRequest] = useState(0);
  ...
  useEffect(() => {
    ...
    if (storedUsername) {
      ...
      axios
        .get(`http://localhost:9000/accounts/Dashboard/${storedUsername}`)
        .then((response) => {
  ...
  useEffect(() => {
    if (requests.length > 0) {
      ...
      setPendingRequest(requests.length - completed);
    }
  }, [requests]);

  const handleViewDetails = (requestId) => {
    axios
      .get(`http://localhost:9000/accounts/viewRequest/${requestId}`)
      .then((response) => {
    ...
    <div className="p-6">
      ...
      {selectedRequest ? (
        <div className="p-6 bg-white shadow rounded-lg mt-6">
          ...
          <p>
            <strong>Course Name:</strong> {selectedRequest.coursename}
          </p>
          <p>
          ...
          <p>
            <strong>Employees Required:</strong> {selectedRequest.requiredemployees}
          </p>
